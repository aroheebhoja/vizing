...
theorem extendPath_maximal :
  ...
  freeColorsOn C ((extendPath C P hne hx hfree hnodup ha hb hneq hcolor).getLast
    (extendPath_nonemptyAx C P hne hx hfree hnodup ha hb hneq hcolor)) := by
  fun_induction extendPath C P hne hx hfree hnodup ha hb hneq hcolor <;> unfold extendPath
  rename_i P _ _ _ _ hnext
  simp_rw [hnext]
  simp [nextVertex] at hnext
  apply free_if_not_exists_nbor at hnext
  · assumption
  · rcases next_eq_a_or_b a b P with hnext | hnext <;> rwa [hnext]
  rename_i hnext _ _ _ ih
  simp_rw [hnext]
  exact ih

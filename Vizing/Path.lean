...
where extendPath (u : Vertex n) (j : Color a) (p : Path n) : Path n :=
  let us := (nbors n G graphsize u).filter (fun v => color a n G C edgecoloring1 (u, v) == j)
  let k := if j == c then d else c
  match h : us with
  | [] => p
  | u'::_ =>
    let p' := (u', u) :: p
    have : ((List.erase (edgeSet n G graphsize) (u', u)).diff p).length < ((edgeSet n G graphsize).diff p).length := by
      have h1 : (u', u) ∉ p := by sorry
      have h2 : (u', u) ∈ edgeSet n G graphsize := by sorry
      have h3 : (u', u) ∈ (edgeSet n G graphsize).diff p := by
        exact List.mem_diff_of_mem h2 h1
      have h4 : (List.diff (edgeSet n G graphsize) p).length > 0 := by
